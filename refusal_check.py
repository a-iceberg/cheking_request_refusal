...
class ReasonResponse(BaseModel):
    reasons: Literal[
        "Не было отказа",
        "Выбранную услугу, ремонт уже оказали, оказывают или точно будут оказывать другие: компания, мастер другой компании, знакомый клиента, сам клиент и тому подобное - сюда НЕ относятся случи, когда другой человек уже выполнил просто диагностику и клиент отказывается от новых работ или же смена исполнителя произошла из-за несовпадений по времени",
        ...
        "Именно техника САМА заработала, неполадка исправилась, проблема решена, не требует вмешательства. Сюда НЕ относится ремонт кем-то другим",
        ...
        "Был осуществлен дублирующий вызов мастера этой же компании по одной и той же услуге, технике по тому же адресу и в то же время - именно повторный вызов после работ в прошлый раз НЕ подходит и сюда НЕ относится",
        ...
        "Вместо ремонта, услуги клиент купил новую технику на замену и обязательно уже оплатил её, ждёт доставку или уже получил - ещё только высказанное намерение купить НЕ относится к этой причине",
                    ...
                    messages = [
                        {
                            "role": "system",
                            "content": """Вы специалист по маркетингу и работе с возражениями, отказами клиентов, а также профессиональный социолог.
Далее представлены диалоги в рамках одной заявки в сервисный центр. Проанализируйте их все и выберите наиболее подходящую причину отказа по данной заявке из списка ниже (отказ - это когда клиент отказался от услуг мастера, часто отказ сопровождается словами Отбой, тогда не надо, не актуально, отмените заявку, не надо приезжать и т.п.), если в диалогах в контексте отказа действительно фигурировало именно то, что вы выбираете, то есть должна быть информация именно про это в разговоре, а НЕ про что-то ещё, пусть и похожее.
